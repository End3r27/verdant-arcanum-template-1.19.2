...
// Add the spell essence slots
    ...
    public ItemStack transferSlot(PlayerEntity player, int index) {
        ...
        if (slot != null && slot.hasStack()) {
            ...
            if (index < LivingStaffItem.MAX_SLOTS) {
                // Move from staff to inventory
                if (!this.insertItem(slotStack, LivingStaffItem.MAX_SLOTS, this.slots.size(), true)) {
                    return ItemStack.EMPTY;
                }
            } else if (slotStack.getItem() instanceof SpellEssenceItem) {
                // Move spell essence from inventory to staff
                if (!this.insertItem(slotStack, 0, LivingStaffItem.MAX_SLOTS, false)) {
                    return ItemStack.EMPTY;
                }
            }
